...
namespace SynapseStudio
{
    public class SynapseHelpers
    {
        ...
        public static List<SchemaMigration.Action> CompareDataTables(string keyColumn, string idColumn, string contextIdColumn, string contextId, List<string> compareColumnList, DataTable dtSource, DataTable dtDestination)
        {
            List<SchemaMigration.Action> ma = new List<SchemaMigration.Action>();
            ...
            foreach (string newCol in notinDest) // new/create action object will have source ids for keyId and context
            {
                ...
                ma.Add(new SchemaMigration.Action(keyColumn, idColumn, row[keyColumn].ToString(), row[idColumn].ToString(), null, dtSource.TableName,
            ...
            foreach (string delCol in notinSrc) // delete action object will have destination ids for keyId and context
            {
                ...
                ma.Add(new SchemaMigration.Action(keyColumn, idColumn, row[keyColumn].ToString(), null, row[idColumn].ToString(), dtSource.TableName,
                                                    MigrationAction.Delete, true, contextIdColumn, contextId, row[contextIdColumn].ToString()));

                //ma.Add(new SchemaMigration.Action(row[idColumn].ToString(), row[keyColumn].ToString(), dtDestination.TableName, MigrationAction.Delete, true, row[contextIdColumn].ToString(), contextIdColumn));
            }
            ...
            foreach (string updCol in inBoth) // update action object will have both source and destination ids for keyId and contextid
            {
                ...
                ma.Add(new SchemaMigration.Action(keyColumn, idColumn, row[keyColumn].ToString(), row[idColumn].ToString(), row[idColumn].ToString(), dtSource.TableName,
                                                    MigrationAction.Update, true, contextIdColumn, srcRow[contextIdColumn].ToString(), row[contextIdColumn].ToString()));


                // ma.Add(new SchemaMigration.Action(row[idColumn].ToString(), row[keyColumn].ToString(), dtDestination.TableName, MigrationAction.Update, true, row[contextIdColumn].ToString(), contextIdColumn));
            }
        ...
        //                List<SchemaMigration.Action> updateActions = entity.attributeActions.Where(x => x.migrationAction == MigrationAction.Update && x.sourceContextId == entity.Id).ToList();
        //                List<SchemaMigration.Action> deleteActions = entity.attributeActions.Where(x => x.migrationAction == MigrationAction.Delete && x.sourceContextId == entity.Id).ToList();
        //                List<SchemaMigration.Action> createActions = entity.attributeActions.Where(x => x.migrationAction == MigrationAction.New && x.sourceContextId == entity.Id).ToList();

        //                foreach (SchemaMigration.Action updateAction in updateActions)
        //                {
        ...
        //                foreach (SchemaMigration.Action deleteAction in deleteActions)
        //                {
        ...
        //                foreach (SchemaMigration.Action createAction in createActions)
        //                {
