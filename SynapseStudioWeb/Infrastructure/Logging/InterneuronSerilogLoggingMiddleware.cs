...
namespace Interneuron.Web.Logging
{
    public class InterneuronSerilogLoggingMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            if (context.Session.GetString(SynapseSession.UserID).IsNotEmpty())
            {
                var userName = context.Session.GetString(SynapseSession.UserID) ?? "StudioWeb User";
                ...
                var userClaimDetails = context.Session.GetString(SynapseSession.UserClaims) ?? null;
                ...
                //using (LogContext.PushProperty("ClientDetails", userClaimDetails))
                //{
