...
{
    public class PatientSearchTermsBuilder : SearchTermsBuilder
    {
        private string _defaultHospitalRefNo;
        private IServiceProvider _provider;

        public PatientSearchTermsBuilder(IServiceProvider _provider)
        {
        ...
        public override Dictionary<string, Func<string, List<SynapseSearchTerm>>> GetSearchTerms()
        {
            return new Dictionary<string, Func<string, List<SynapseSearchTerm>>>()
            {
                    ...
                    { "_lastUpdated", (paramVal)=>
                        {
                            return new List<SynapseSearchTerm>() { new SynapseSearchTerm($"personData.{nameof(entitystorematerialised_CorePerson.Createddate)}", DateTimeSearchExpressionProvider.GreaterThanEqualToOperator, paramVal.Substring(2,paramVal.Length-2), new DateTimeSearchExpressionProvider()) };
                        }
                    },
        ...
        {
            return new List<SynapseSearchTerm>() { new SynapseSearchTerm($"personData.{nameof(entitystorematerialised_CorePerson.PersonId)}", StringSearchExpressionProvider.EqualsOperator, paramVal, new StringSearchExpressionProvider())
            };
        }
    }
}
