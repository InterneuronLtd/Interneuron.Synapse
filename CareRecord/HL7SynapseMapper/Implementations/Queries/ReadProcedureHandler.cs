...
{
    internal class ReadProcedureHandler : BaseReadQueryHandler
    {
        private const string SearchKeyIdentifier = "search_procedure";

        private IServiceProvider _provider;
        private IReadOnlyRepository<entitystorematerialised_CoreProcedure> _coreProcedureRepo;
        private IFHIRParam _fhirParam;

        public ReadProcedureHandler(IServiceProvider provider, IMapper mapper, IReadOnlyRepository<entitystorematerialised_CoreProcedure> coreProcedureRepo, IGenericSearchRepository genericSearchRepo) : base(provider, mapper, genericSearchRepo)
        {
            this._provider = provider;
            this._coreProcedureRepo = coreProcedureRepo;
        }
        ...
        private ResourceData NoResultsHandler(IFHIRParam fhirParam)
        {
            ...
            var storeCoreProcedureResult = CheckInEntityStore(fhirParam);

            if (storeCoreProcedureResult == null || storeCoreProcedureResult.ProcedureId == null) return resourceData;

            if (storeCoreProcedureResult.Recordstatus == 2) // Procedures in Deleted State
            {
                resourceData.Resource = null;
                resourceData.DeletedDate = storeCoreProcedureResult.Createddate;
        ...
        private entitystorematerialised_CoreProcedure CheckInEntityStore(IFHIRParam fhirParam)
        {
            return _coreProcedureRepo.ItemsAsReadOnly.Where(e => e.ProcedureId == fhirParam.ResourceId).OrderByDescending(x => x.Sequenceid).FirstOrDefault();
