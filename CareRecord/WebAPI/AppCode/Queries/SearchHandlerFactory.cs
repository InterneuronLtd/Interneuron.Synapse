...
namespace Interneuron.CareRecord.API.AppCode
{
    ...
    public class SearchHandlerFactory
    {
        ...
        public SearchHandlerFactory(System.IServiceProvider provider)
        {
            this._provider = provider;
        }
        public ISearchHandler GetHandler(string actionName)
        {
            if (actionName.EqualsIgnoreCase("search"))
            {
                return this._provider.GetService(typeof(SearchHandler)) as SearchHandler;
            }
            return null;
        }
    }
